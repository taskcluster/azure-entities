...
Entity.setup = function(options) {
  ...
  subClass.prototype.__tableName  = tableName;
  ...
  [
    ...
    aux[method] = Promise.denodeify(client[method].bind(client, tableName));
  });
  // Auxiliary method for queryEntities, which is a special-case as it carries a
  // continuation token as third parameter in the callback.
  aux.queryEntities = function(options) {
    return new Promise(function(accept, reject) {
      client.queryEntities(tableName, options, function(err, data, token) {
        if (err) {
          return reject(err);
        }
        return accept({
...
Entity.scan = function(conditions, options) {
  ...
  // Construct a list of filters from conditions using operators
  var filters = _.map(conditions, function(op, property) {
    ...
    if (!(op instanceof Entity.op)) {
      op = Entity.op.equals(op);
    }

    // Let the operator construct the filter
    return op.filter(type, property);
  }).filter(function(filter) {
    return filter !== undefined && filter !== null;
  });


  // If we have partitionKey and rowKey we should add them to the filter
  if (partitionKey !== undefined) {
    filters.unshift(['PartitionKey', 'eq', partitionKey].join(' '));
  }
  if (rowKey !== undefined) {
    filters.unshift(['RowKey', 'eq', rowKey].join(' '));
  }

  // Construct fake query object for azure-table-node
  var query = {_query: filters.join(' and ')};
  ...
  if (options.handler) {
    var handleResults = function(results) {
      return results.then(function(data) {
        return Promise.all(data.entries, function(item) {
   ...
   - Filters on additional properties (not covered by rowkey)
{
  ...
  // maxNumber to call in parallel (you can abort by failing the promise returned)
