 ...
 *   encryptionKey:     "...",              // Key for encrypted properties
...
Entity.setup = function(options) {
  ...
  if (subClass.prototype.__hasEncrypted) {
    assert(typeof(options.encryptionKey) === 'string',
           "encryptionKey is required when a property is encrypted in any " +
           "of the schema versions.");
    var secret  = new Buffer(options.encryptionKey, 'base64');
    assert(secret.length === 32, "encryptionKey must be 32 bytes in base64");
    subClass.prototype.__cryptoKey = secret;
  } else {
    assert(!options.encryptionKey, "Don't specify options.encryptionKey when " +
