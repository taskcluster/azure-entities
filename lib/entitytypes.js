 ...
 *
 * If `true` the `encryptionKey` will be given as 3rd and 2nd parameter for
 ...
 *
 * Will serialize `value` to `target` object, given `encryptionKey` if this
 * is an encrypting type (one that has `isEncrypted: true`) the type must
 * encrypted the data with `encryptionKey` before saving it to target.
 */
BaseType.prototype.serialize = function(target, value, encryptionKey) {
 ...
 * must decrypted the data with `encryptionKey` before deserializing it.
 */
BaseType.prototype.deserialize = function(source, encryptionKey) {
  throw new Error("Not implemented");
};
...
BaseBufferType.prototype.toBuffer = function(value, encryptionKey) {
  throw new Error("Not implemented");
};

/** Transform value from buffer */
BaseBufferType.prototype.fromBuffer = function(buffer, encryptionKey) {
  throw new Error("Not implemented");
};

BaseBufferType.prototype.serialize = function(target, value, encryptionKey) {
  value = this.toBuffer(value, encryptionKey);
...
BaseBufferType.prototype.deserialize = function(source, encryptionKey) {
  ...
  return this.fromBuffer(Buffer.concat(chunks), encryptionKey);
};
...
EncryptedBaseType.prototype.toBuffer = function(value, encryptionKey) {
...
EncryptedBaseType.prototype.fromBuffer = function(buffer, encryptionKey) {
